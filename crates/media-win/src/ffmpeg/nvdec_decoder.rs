//! Windows FFmpeg `hevc_cuvid` NVDEC decoder adapter (8-bit HEVC Main profile).
//! Mirrors `crates/media-ffmpeg/src/hevc_nvdec_decoder.rs` for Linux; key
//! differences:
//!   - Uses `rusty_ffmpeg_win` instead of `rusty_ffmpeg`.
//!   - Error type is `MediaError` (not `FfmpegError`).
//!   - No `CudaHwDevice` wrapper — opens the CUDA HW device inline via
//!     `av_hwdevice_ctx_create(AV_HWDEVICE_TYPE_CUDA, ...)`.
//!   - Emits `Nv12Frame` (owned CPU NV12 planes).
//!
//! Cargo cfg gate: `#[cfg(feature = "media-win-ffmpeg-nvdec-any")]`.
...
mod inner {
    ...
    use rusty_ffmpeg_win::ffi::{
        ...
        av_frame_unref, av_hwdevice_ctx_create, av_hwframe_transfer_data as hw_download,
        av_packet_alloc, av_packet_free, av_packet_unref, avcodec_alloc_context3,
        avcodec_find_decoder_by_name, avcodec_free_context, avcodec_open2, avcodec_receive_frame,
        avcodec_send_packet, AVBufferRef, AVCodecContext, AVDictionary, AVFrame, AVPacket,
    ...
    pub struct HevcNvdecFfmpegDecoderWindowsConfig {
        pub width: u32,
        pub height: u32,
        /// CUDA device index. Reserved for future multi-GPU selection; currently
        /// unused (device picked by `CUDA_VISIBLE_DEVICES` env / default 0).
        pub cuda_device_index: Option<u32>,
    }

    impl Default for HevcNvdecFfmpegDecoderWindowsConfig {
        fn default() -> Self {
            Self {
                width: 1920,
                height: 1080,
    ...
    pub struct HevcNvdecFfmpegDecoderWindows {
    ...
    // SAFETY: HevcNvdecFfmpegDecoderWindows owns its libavcodec + CUDA resources
    // exclusively via NonNull pointers; never aliased; decode pipeline runs
    // single-threaded (parallel to the encoder adapter's reasoning).
    unsafe impl Send for HevcNvdecFfmpegDecoderWindows {}

    impl HevcNvdecFfmpegDecoderWindows {
        pub fn new(cfg: HevcNvdecFfmpegDecoderWindowsConfig) -> Result<Self, MediaError> {
            ...
            unsafe {
                ...
                // output. AV_CODEC_FLAG_LOW_DELAY (1<<19) makes hevc_cuvid emit
                // each frame as soon as it is decoded instead of holding it for
                // reorder. Defined locally to avoid depending on the FFI crate
                // re-exporting it.
            ...
            // delay via the private `delay=0` option (belt-and-suspenders with
            // the LOW_DELAY flag above); cuvid ignores option keys it does not
        ...
        pub fn backend_name(&self) -> &'static str {
            "ffmpeg-nvdec-hevc-win"
        }
    }

    impl Drop for HevcNvdecFfmpegDecoderWindows {
        fn drop(&mut self) {
    ...
    /// at `data[1]`) into an owned `Nv12Frame`.
    ///
    /// # Safety
    ...
    unsafe fn copy_nv12_planes(
        ...
        // SAFETY: caller guarantees y_ptr/uv_ptr are readable for the declared byte counts;
        // the dst Vecs are freshly allocated and owned.
        unsafe {
    ...
    mod tests {
        ...
        fn context_creation_no_gpu() {
            ...
            // If hevc_cuvid IS available, new() might succeed or fail with a
            // CUDA/driver error — both are fine; the requirement is no panic.
        }

        #[test]
        fn default_config_fields() {
            let cfg = HevcNvdecFfmpegDecoderWindowsConfig::default();
...
#[cfg(feature = "media-win-ffmpeg-nvdec-any")]
pub use inner::{HevcNvdecFfmpegDecoderWindows, HevcNvdecFfmpegDecoderWindowsConfig};
