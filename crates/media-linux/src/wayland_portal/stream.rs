                ...
                let _listener = stream
                    ...
                    .param_changed({
                        ...
                        move |stream, _ud, id, param| {
                            ...
                            // Diagnostic: dump POD bytes + structural summary
                            // so we can see exactly what mutter delivered when
                            // parse() fails downstream. Tied to the
                            // P5B-2a-successor smoke on N100 (2026-05-13)
                            // where parse() rejected mutter's POD as
                            // "MediaType is not Video".
                            {
                                ...
                                tracing::info!(
                                    ...
                                    pod_hex = %bytes
                                        ...
                                        .map(|b| format!("{:02x}", b))
                                        .collect::<String>(),
                                    "param_changed: pre-parse POD bytes (first 64)"
                                );
                                // Also try to deserialize into Value and log
                                // the Object type + property keys so we see
                                // what fields are present.
                                use pipewire::spa::pod::deserialize::PodDeserializer;
                                use pipewire::spa::pod::Value;
                                if let Ok((_n, value)) =
                                    PodDeserializer::deserialize_any_from(bytes)
                                {
                                    match &value {
                                        Value::Object(obj) => {
                                            ...
                                            tracing::info!(
                                                obj_type = obj.type_,
                                                obj_id = obj.id,
                                                num_props = obj.properties.len(),
                                                prop_keys = ?keys,
                                                "param_changed: POD is Object"
                                            );
                                            // Also dump first-level Value
                                            // variant of each property for
                                            // sleuthing.
                                            for prop in &obj.properties {
                                                let vname: &str = match &prop.value {
                                                    Value::Bool(_) => "Bool",
                                                    Value::Id(_) => "Id",
                                                    Value::Int(_) => "Int",
                                                    Value::Long(_) => "Long",
                                                ...
                                                tracing::info!(
                                                    key = prop.key,
                                                    flags_bits = prop.flags.bits(),
                                                    value_variant = vname,
                                                    "  prop"
                                                );
                                                // For Id values, log the inner u32 so we
                                                // can confirm MediaType=video etc.
                                                if let Value::Id(id_val) = &prop.value {
                                                    tracing::info!(
                                                        key = prop.key,
                                                        id_value = id_val.0,
                                                        "    id-value"
                                        ...
                                        other => {
                                            tracing::warn!(
                                                pod_variant = ?std::mem::discriminant(other),
                                                "param_changed: POD is NOT Object"
                                            );
                                        }
                                    }
                                } else {
                                    tracing::warn!(
                                        "param_changed: PodDeserializer::deserialize_any_from \
                                         failed"
                                    );
                                }
                            }
