...
async fn viewer_session_start_timeout() {
    ...
    let err = rendezvous_as_viewer(
        RendezvousConfig { url, host_id: "ghost".into(), timeout: Duration::from_millis(300), stun_url: None },
        local,
    ).await.unwrap_err();
    match err {
...
async fn viewer_peer_candidate_timeout() {
    ...
    let err = rendezvous_as_viewer(
        RendezvousConfig { url, host_id: "h1".into(), timeout: Duration::from_secs(1), stun_url: None },
        local,
    ).await.unwrap_err();
    match err {
