//! NVDEC HEVC Main10 decoder via `hevc_cuvid`. Sibling of `hevc_nvdec_decoder.rs`
//! (8-bit). Key differences:
...
//! (CI grep guard; same as 8-bit NVDEC decoder).
...
pub struct HevcNvdecMain10FfmpegDecoderConfig {
    pub width: u32,
    pub height: u32,
...
impl Default for HevcNvdecMain10FfmpegDecoderConfig {
    fn default() -> Self {
        Self {
            width: 1920,
            height: 1080,
...
pub struct HevcNvdecMain10FfmpegDecoder {
    #[allow(dead_code)]
    device: CudaHwDevice,
...
// SAFETY: HevcNvdecMain10FfmpegDecoder owns its libavcodec + CUDA resources
// exclusively via NonNull pointers; never aliased; decode pipeline runs
// single-threaded.
unsafe impl Send for HevcNvdecMain10FfmpegDecoder {}

impl HevcNvdecMain10FfmpegDecoder {
    pub fn new(cfg: HevcNvdecMain10FfmpegDecoderConfig) -> Result<Self, FfmpegError> {
        ...
        tracing::info!(
            ...
            event = "decoder_ready",
            backend = "ffmpeg-nvdec-hevc-main10",
            codec = "h265",
            profile = "main10",
            bitdepth = 10,
        );
...
impl HevcDecoderBackend10 for HevcNvdecMain10FfmpegDecoder {
    ...
    fn backend_name(&self) -> &'static str {
        "ffmpeg-nvdec-hevc-main10"
    }
}

impl Drop for HevcNvdecMain10FfmpegDecoder {
    fn drop(&mut self) {
...
mod tests {
    ...
    #[test]
    #[ignore = "requires NVIDIA dGPU with hevc_cuvid Main10 decode"]
    fn constructs_with_real_nvdec_main10() {
        let cfg = HevcNvdecMain10FfmpegDecoderConfig::default();
        let dec = HevcNvdecMain10FfmpegDecoder::new(cfg).expect("decoder created");
        assert_eq!(dec.backend_name(), "ffmpeg-nvdec-hevc-main10");
    }
}
