...
fn main() {
    ...
    // this, latency-bench's `#[cfg(prdt_nvdec_bindings)]` guards always
    // resolve to the fallback branch — even though media-win itself has
    // the bindings — because cfgs from build.rs only apply to the crate
    // that emits them.
